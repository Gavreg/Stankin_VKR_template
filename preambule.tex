...
%   Чтобы было проще интегировать изменения шаблона из гита.
...
\newfontfamily{\cyrillicfontsf}{Arial}[Mapping=tex-text]
\newfontfamily{\cyrillicfonttt}{Courier New}[Mapping=tex-text]
...
\setlength{\parskip}{0pt plus 3pt minus 2pt} % разрешим latex чучуть увеличивать или уменьшать межабзацное расстрояние.

\usepackage[all]{nowidow}
\setnowidow[2] % минимум 2 строки вверху новой страницы
\setnoclub[2]  % минимум 2 строки внизу старой страницы
...
\usepackage{array}
\usepackage{makecell} %для сложных ячеек
\usepackage{multirow} %для ячеек в несколько строк
...
\def\everytablespatch{%
	\ifdefined\insidetable%
		\ifDebug%
			\color{red}%
		\fi%
		\fontsize{12pt}{14pt}%
...
\AtBeginEnvironment{tabular}{\everytablespatch}
\AtBeginEnvironment{longtable}{\everytablespatch}
\AtBeginEnvironment{tabularx}{\everytablespatch}
...
\counterwithin{equation}{section} %счетчик формул по главам
...
\def\baselinestretch{1.425}   %17*1.425 = 24.225  как в ворде! замерял линейкой
...
\captionsetup[figure]{
	...
	font={onehalfspacing},
	%skip=1.2em, 
	belowskip=-1em-3pt, %сдвиг границы плавающего объекта рисунка на базовую линию надписи 
	%aboveskip=0pt 
}                       
...
\floatplacement{figure}{!htb} %в месте вызова или внизу станицы
\floatplacement{table}{!htb} %в месте вызова или внизу станицы
\floatplacement{listing}{!htb} %в месте вызова или внизу станицы
...
\setlength{\floatsep}{1em plus 2pt minus 2pt}     % между float'ами
\setlength{\textfloatsep}{11pt+24pt} % между float'ами и текстом
\setlength{\intextsep}{11pt+24pt}    % для float'ов внутри текста

%\AtEndEnvironment{figure}{\vspace{-0.75em}}
\AtEndEnvironment{table}{\vspace{-0.75em}}
\AtEndEnvironment{listing}{\vspace{-2em}}
...
\newenvironment{VKRlongtable}{%
	...
	{\vspace{\intextsep-14pt}\endgroup}
...
%о как долго я искал это решение!
...
\setlist[enumerate]{
	...
	leftmargin=1.0cm,     
	labelsep=0.65ex,
	itemindent=\widthof{\ref{enum-\EnumitemId}}+\the\labelsep,
	label*=\arabic*.,	
}
...
\usepackage{refcount}  %всякие команды для работы с ссылками, например \getpagerefnumber
\usepackage{metalogo}
...
\ifDebug
	...
	\newcommand{\botfigrule}{{\color{green}\rule{\paperwidth}{0.5pt}}}
	\newcommand{\topfigrule}{{\color{green}\rule{\paperwidth}{0.5pt}}}
...
\ifDebug
	\usepackage{showkeys}
		...
		% Переопределение формата с жестким приведением размера для полей
		\renewcommand*\showkeyslabelformat[1]{%
			\fbox{%
				\parbox[t]{2.4cm}{% Ограничиваем ширину плашки на поле
					\normalfont\tiny\ttfamily % Устанавливаем самый мелкий шрифт
					\raggedleft % Выравнивание по левому краю, чтобы длинные метки переносились
					\hyphenpenalty=0 % Разрешаем перенос слов в названиях меток
					#1%
				}%
			}%
		}
