...
\appto\appendix{%
	...
	{0em} % Расстояние между номером и текстом
	{\\} % Код перед заголовком
	[\vspace{3ex}] % Расстояние после заголовка (2 интервала = 3ex при 1.5)
...
%отключаем счеткики для приложений
\appto{\appendix}{%
	%\undopatch{\figure}	
}
...
%Подпись табл. слева
\captionsetup[listing]{justification=raggedleft,labelsep=dot, format=plain, position=above, singlelinecheck=false,skip=-0 em} 


%В рисунках и таблиц приложений используем префикс П
\appto\appendix{%
	\renewcommand{\thefigure}{П\thesection.\arabic{figure}}
	\renewcommand{\thetable}{П\thesection.\arabic{figure}}
}
